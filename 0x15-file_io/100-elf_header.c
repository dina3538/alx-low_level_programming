 ...
 * Description: If the file is not file - exit code 98.
 */
...
/**
 * p_entry - Prints the entry point
 * @e_entry: The address
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	r = read(g, header, sizeof(Elf64_Ehdr));
	if (r == -1)
	{
		free(header);
		close_elf(g);
